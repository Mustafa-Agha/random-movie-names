...
describe("random-movie-names", function () {
  it("should return one name when called with no arguments", function () {
  ...
  it("should return between 5 and 10 names when called with min: 5 and max: 10", function () {
  ...
  it("should return 5 space separated names when join is used with exactly: 5", function () {
  ...
  it("should return 5 concatenated names when join is used with an empty string and exactly: 5", function () {
    let phrase = randomMovieNames({ exactly: 5, join: "" });
    assert.ok(typeof phrase === "string", "result is a string");
    assert.ok(phrase.match(/\w/), "result contains text, no spaces");
  });
  it("should return 5 names when called with exactly: 5 and join: false", function () {
    const movieNames = randomMovieNames({ exactly: 5, join: false });
    assert.ok(movieNames.length === 5, "contains 5 elements");
  });
  it("should return 5 names when called with exactly: 5 and join: null", function () {
    const movieNames = randomMovieNames({ exactly: 5, join: null });
    assert.ok(movieNames.length === 5, "contains 5 elements");
  });
  it("should only return names within the maxLength", function () {
    const maxNameSize = 4;
  ...
  it("should return 5 space separated names for each string if namesPerString is set to 5 and exactly > 1", function () {
  ...
  it("should reuturn 5 names separated by a separator for each string if namesPerString > 1, separator is defined as a string and exactly > 1", function () {
    const separator = "-";
